...
function resolvedActivity(row) { const fixedAttempt = row.resolved_provider ? { provider: row.resolved_provider, model: row.resolved_model || '', failure_class: row.error_text, latency_ms: row.latency_ms, http_status: row.http_status, error_message: row.error_message, error_body: row.error_body, error_body_truncated: row.error_body_truncated, result: row.http_status >= 200 && row.http_status < 300 ? 'success' : 'failed' } : null;   const resolved = row.fallback_used ? '' : fixedAttempt ? (row.attempt_count > 1 ? '' : `<div class="attempt-sequence">${activityAttempt(fixedAttempt, 0)}</div>`) : '';
   const sequence = (row.fallback_used || !fixedAttempt || row.attempt_count > 1) ? attemptSequence(row) : ''; const errorHover = [row.error_message, row.latency_ms ? `Latency: ${row.latency_ms} ms` : ''].filter(Boolean).join(' · '); const error = sequence ? '' : (row.error_text ? `<span class="error-text"${errorHover ? ` title="${h(errorHover)}"` : ''}>${h(row.error_text)}</span>` : ''); const requestBody = activityBodyButton('Client request body', row.request_body, row.request_body_truncated); return `${resolved}${sequence}${error}${requestBody}`; }
